...
Runtime does not mean how long an algorithm takes to run on any individual computer, but rather the number of operations in the worst case. We focus on how quickly the runtime of an algorithm grows in respect to its input size.
...
In my paper with Brian An, *Uncovering Neighborhood-level Portfolios of Corporate Single-Family Rental Holdings and Equity Loss*, our priority was developing a simple method to accurately quantify ownership scale, rather than identify the portolfios of specific owners [@jua]. However, this is possible with an additional querying method. This section details these methods.
...
::: {.callout-note title="Table 2. Sample of Corporate Owners and Associated Subsidaries" appearance="minimal"}
...
The challenge comes from cases where both the owner names and addresses are different but they represent the same owner. For instance, how is it possible that any algorithm or model can correctly classify "Jeff 1 LLC" as a subsidary Amherst Capital if the owner address is different? 
...
- A sample of labeled data (ground truth) across multiple metros areas to train and/or test models against. This is related to the data crowdsourcing mentioned previously.
