...
It is unlikely that the marginal improvement from categeory 2 to 3 is worth the additional effort. Training a deep learning model may be an exception, as it only requires extensive infrastructure and expertise once during training. We will discuss this further later.
...
These functions typically compare each string to every string using a similarity metric, and group strings that are above a certain threshold.
...
::: {.callout-note title="Example" appearance="minimal"}
Please refer to Dr. Brian An's work (site)
:::
...
Training a deep learning model typically requires dedicated GPUs, but once the model is trained, it can be used much more easily and quickly.

Should not be trained on full data.
